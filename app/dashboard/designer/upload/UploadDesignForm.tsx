...
export default function UploadDesignForm() {
  ...
  const handleSubmit = async () => {
    ...
    if (!user) {
      setSubmitting(false)
      return setError('You must be logged in to upload a design.')
    }

    // TODO: upload the actual file to Cloudinary and use the returned URL
    // here instead of just storing the filename as a placeholder.
  ...
  return (
    <div style={s.page}>
      ...
      <div style={s.body}>
        ...
        {error && (
          <div style={{ background: '#FEF2F2', color: '#991B1B', borderRadius: 8, padding: '10px 14px', fontSize: 13, marginBottom: 'var(--space-6)' }}>
            {error}
          </div>
        )}
