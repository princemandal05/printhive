...
export default async function BuyerDashboard() {
  ...
  const s: Record<string, React.CSSProperties> = {
    ...
    badge: { background: '#ECFDF5', color: '#10B981', borderRadius: 99, padding: '4px 12px', fontSize: 12, fontWeight: 600 },
  ...
  return (
    <div style={s.page}>
      ...
      <div style={s.body}>
        <div style={s.welcome}>Welcome to PrintHive 👋</div>
        <div style={s.sub}>{user.email} · Buyer account</div>

        <div style={s.grid}>
          {[
            { val: '0', label: 'Orders placed' },
            { val: '0', label: 'Delivered' },
            { val: '0', label: 'In progress' },
            { val: '0', label: 'Saved designs' },
          ].map(({ val, label }) => (
        ...
        <div style={s.section}>
          ...
          <div style={s.emptyState}>
            ...
            <div>Browse the marketplace and place your first 3D print order</div>
          </div>
        </div>
