...
export default function PrinterOwnerClient({ user, initialPrinters, initialOrders }: Props) {
  ...
  // Real-time synchronization for new and updated print orders
  useEffect(() => {
    async function reloadOrders() {
      try {
        ...
        if (dbOrders) {
          const mapped: OrderJob[] = dbOrders.map((o: any) => {
        ...
        console.warn('Live orders reload note:', e)
      }
    }

    const channel = supabase
      .channel(`printer-orders-${user.id}`)
      .on(
        'postgres_changes',
        { event: '*', schema: 'public', table: 'orders', filter: `printer_owner_id=eq.${user.id}` },
        () => {
          reloadOrders()
        }
      )
    ...
    const interval = setInterval(reloadOrders, 4000)
