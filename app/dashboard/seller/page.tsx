...
export default async function SellerDashboard() {
  ...
  const s: Record<string, React.CSSProperties> = {
    ...
    badge: { background: '#F5F3FF', color: '#8B5CF6', borderRadius: 99, padding: '4px 12px', fontSize: 12, fontWeight: 600 },
    ...
    btn: { background: '#8B5CF6', color: '#fff', border: 'none', borderRadius: 8, padding: '10px 20px', fontSize: 14, fontWeight: 600, cursor: 'pointer' },
    ...
    <div style={s.page}>
      <nav style={s.nav}>
        ...
        <div style={s.navRight}>
          <span style={s.badge}>Seller</span>
      ...
      <div style={s.body}>
        ...
        <div style={s.sub}>{user.email} · Seller account</div>

        <div style={s.grid}>
          {[
            { val: '0', label: 'Products listed' },
            { val: '0', label: 'Orders this month' },
            { val: '₹0', label: 'Revenue' },
            { val: '0', label: 'Low stock alerts' },
          ].map(({ val, label }) => (
        ...
        <div style={s.section}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
            <div style={s.sectionTitle}>My Store</div>
            <Link href="/dashboard/seller/products/new" style={{ ...s.btn, display: 'inline-block', textDecoration: 'none' }}>+ Add Product</Link>
          </div>
          <div style={s.emptyState}>
            <div style={{ fontSize: 32, marginBottom: 8 }}>🏬</div>
            <div style={{ fontWeight: 500, color: '#475569', marginBottom: 4 }}>No products listed yet</div>
            <div>Add your first ready-made product to open your store</div>
          </div>
        </div>

        <div style={s.section}>
          <div style={s.sectionTitle}>Incoming Orders</div>
          <div style={s.emptyState}>Orders for your products will appear here</div>
        </div>

        <div style={s.section}>
          <div style={s.sectionTitle}>Discounts & Promotions</div>
          <div style={s.emptyState}>Create a discount code to boost sales</div>
        </div>
      </div>
    </div>
  )
